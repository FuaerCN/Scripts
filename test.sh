...
aria2ng_new_ver=$(wget -qO- https://api.github.com/repos/mayswind/AriaNg/releases/latest | grep -o '"tag_name": ".*"' | sed 's/"//g' | sed 's/tag_name: //g') && echo ${Ver}
...
Install_caddy(){
	...
	if [[ ${bit} == "i386" ]]; then
		wget -O "caddy_linux.tar.gz" "https://caddyserver.com/download/linux/386?license=personal" && caddy_bit="caddy_linux_386"
	elif [[ ${bit} == "i686" ]]; then
		wget -O "caddy_linux.tar.gz" "https://caddyserver.com/download/linux/386?license=personal" && caddy_bit="caddy_linux_386"
	elif [[ ${bit} == "x86_64" ]]; then
		wget -O "caddy_linux.tar.gz" "https://caddyserver.com/download/linux/amd64?license=personal" && caddy_bit="caddy_linux_amd64"
	else
	...
	rm -rf "init/"
	chmod +x caddy
	wget https://raw.githubusercontent.com/FuaerCN/sh/master/Caddyfile
	
}

Install_aria2(){
    wget https://raw.githubusercontent.com/FuaerCN/sh/master/Caddyfile && mv /root/.aria2
	wget https://raw.githubusercontent.com/FuaerCN/sh/master/Caddyfile && mv /root/.aria2
	wget https://raw.githubusercontent.com/FuaerCN/sh/master/Caddyfile && mv /root/.aria2
	wget https://raw.githubusercontent.com/FuaerCN/sh/master/Caddyfile && mv /root/.aria2
...
Install_aria2ng(){
	...
	wget ${aria2ng_download_http} && unzip aria-ng-${aria2ng_new_ver}.zip
}

Install_oneindex(){
    cd /home/ && wget -O oneindex.zip https://codeload.github.com/donwa/oneindex/zip/master && unzip oneindex.zip
